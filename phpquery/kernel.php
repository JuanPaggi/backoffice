...
class _
{
	...
	static protected $bypassController = null;
    ...
    {
		if(!empty(self::$bypassController)) return null;
    ...
    {
		if(!empty(self::$bypassController)) { $action = self::$bypassController; $loadFooter = false; }
	...
	static public function onDbFail()
	{
		if(!empty(coreData::$default_500_controller))
		{
			self::$bypassController = coreData::$default_500_controller;
		} else {
			echo '<h2>PHPQuery Error 500</h2>';
