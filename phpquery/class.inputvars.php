...
define('FILTER_DELETE', 1);
define('FILTER_TO_EOL', 2);
define('FILTER_TO_SOL', 3);
define('USE_REGEX', 4);
...
class objectVar
{
    ...
    public function __construct($val)
    {
		...
		if(is_array($val))
		{
			$this->no_br = array_map('htmlentities' ,$val);
			$this->val = array_map('nl2br', $this->no_br);
			$this->toOut = array_map('trim', $this->val);
		}
        else
		{
    ...
    public function seo()
    {
        ...
        $url = $this->toOut;
        $url = html_entity_decode(html_entity_decode(strtolower($url)));
