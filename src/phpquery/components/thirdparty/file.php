...
Class File
{
	...
	public function uploadPhoto($nombre_campo, $carpeta, $uniqid = false, $namecrypt = false, $extension_valida = array('jpeg', 'png', 'gif', 'jpg'), $file_type = null, $kbmax = 0, $dir = true)
	{
		try{
			$finfo = new finfo(FILEINFO_MIME_TYPE);
			if (false === $ext = array_search(
				$finfo->file($_FILES[$nombre_campo]['tmp_name']),
				array(
					'jpg' => 'image/jpeg',
					'png' => 'image/png',
					'gif' => 'image/gif',
			...
			return $this->upload($nombre_campo, $carpeta, $uniqid, $namecrypt, $extension_valida, $file_type, $kbmax, $dir);
		} catch(Exception $e) {
			$this->error = true;
			$this->error_msg = $e->getMessage();
			return false;
	...
	public function upload($nombre_campo, $carpeta, $uniqid = false, $namecrypt = false, $extension_valida = array('rar'), $file_type = null, $kbmax = 0, $dir = true)
	{
		try 
		{
			...
			$this->name = str_replace(' ', '-', $id).'_'.trim($this->name);
			// verificamos si la extensi�n es v�lida
			$valido = false;
			for($i = 0; $i<count($extension_valida); $i++)
				if($this->validate_extension($this->name, $extension_valida[$i]))
					$valido = true;
			if(!$valido) throw new exception('El archivo es de una extensi�n inv�lida :/');
			// verificamos si el tipo de archivo es valido
			if(!empty($file_type) && $this->type!=$file_type) throw new exception('El tipo de archivo es inv�lido');
			// verificamos si tiene el tama�o correcto
			...
			// retornamos el directorio donde se guard�
			if($dir)
